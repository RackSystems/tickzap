...
- [ ] **#4 WebSocket via Redis pub/sub** — `websocket.ts` guarda clientes em `Map` em memória, mas o `worker.ts` é processo separado: os broadcasts do job de IA nunca chegam. Migrar para pub/sub no Redis (elimina o `redis.keys()` O(n) em `broadcastToWatchingConversation` e destrava réplicas). Remover userId mockado hardcoded; reativar init comentado em `index.ts`. *(depende de #1)*
...
- [ ] **#7 Índices e uniques no schema** — sem nenhum `@@index`; adicionar em `Message.conversationId`, `Conversation(contactId, channelId)`, `Conversation.status`, `Contact(remoteJid, channelId)`. Revisar `@unique` globais em `Contact.phone/remoteJid/email` (conflitam com multi-canal → `@@unique([remoteJid, channelId])`).
- [ ] **#8 Paginação no `ConversationService.index`** — declara `page/pageSize` mas não usa; retorna todas as conversas com 4 includes. Implementar skip/take + metadados. Corrigir retorno `any[]`.
- [ ] **#9 Conversation fechada no webhook** — `WebhookService` anexa mensagem a qualquer conversa, inclusive CLOSED. Buscar conversa com `status != CLOSED`; se não existir, criar. Tratar idempotência (reenvio do webhook → `create` da Message com mesmo id estoura P2002).
...
- [ ] **#11 Validação de entrada + remover no-ops** — Channels e Messages sem validator; criar seguindo `UserValidator`. Remover `handleValidation` órfão em `conversations/:id` show/destroy.
...
- [ ] **#15 Testes** — configurar `bun test`; cobrir `AuthService`, `WebhookService.handleMessagesUpsert`, `MessageService.sendMessage`, `ConversationService`. CI (lint + test).
...
- [ ] `ContactInterface.ts` é código morto (usa-se o tipo `Contact` do Prisma) — apagar; revisar os outros `interfaces/`. `TicketInterface.ts` ganhou novo propósito (chamado de suporte) — implementar quando o módulo for criado.
...
- [x] **channels**
- [x] **agents**
- [x] **conversations** — central, consumido por messages/webhook
...
- [ ] Criar módulo `tickets/` (Controller, Service, Validator, routes) e registrar `/tickets` em `api.ts` quando o schema estiver implementado. Ver [plan.md](plan.md).
